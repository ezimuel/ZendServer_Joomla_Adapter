...
class JCacheStorageZendserverdisk extends JCacheStorage
{
	...
	public function get($id, $group, $checkTime = true)
	{
		...
		return zend_disk_cache_fetch($group . '::' . $cache_id);
	}
	/**
	...
	public function store($id, $group, $data)
	{
		...
		return zend_disk_cache_store($group . '::' . $cache_id, $data, $this->_lifetime);	
	}
	/**
	...
	{
		$cache_id = $this->_getCacheId($id, $group);
        $this->updateGroup($group, -1 * strlen(serialize(zend_disk_cache_fetch($group . '::' . $cache_id))));		
		return zend_disk_cache_delete($group . '::' . $cache_id);
	}
    /**
    ...
    {
        return zend_disk_cache_fetch(self::CACHE_ZS_SYSTEM . '::' . self::CACHE_ZS_GROUPS);
    }
    /**
    ...
    protected function updateGroup($group, $size = 0)
    {
        ...
        return zend_disk_cache_store(self::CACHE_ZS_SYSTEM . '::' . self::CACHE_ZS_GROUPS, $groups); 
    }
    /**
	...
	public function clean($group, $mode = null)
	{
        if ($mode == 'group') {
            ...
            return zend_disk_cache_clear($group);
        } else {
            $groups = $this->getGroups();
            foreach ($groups as $key => $value) {
                if ($key != $group) {
                    $this->removeGroup($key);
                    $result = zend_disk_cache_clear($key);
                    if (false === $result) {
                        return false;
                    }
                }
	...
	public function lock($id, $group, $locktime)
	{
		...
		$data_lock = zend_disk_cache_store($group . '::' . $cache_id, 1, $locktime);
